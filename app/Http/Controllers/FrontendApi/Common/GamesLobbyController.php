...
use App\Http\Requests\Frontend\Common\GamesLobby\InGameTxBalanceOutRequest;
...
use App\Http\SingleActions\Frontend\Common\GamesLobby\InGameTxBalanceOutAction;
...
class GamesLobbyController extends Controller
{
     ...
     * 取钱到平台 目前只有 IM 有
     *
     * @param InGameTxBalanceOutAction  $action  Action.
     * @param InGameTxBalanceOutRequest $request Request.
     * @return JsonResponse
     * @throws \Exception Exception.
     */
    public function inGameTxBalanceOut(
        InGameTxBalanceOutAction $action,
        InGameTxBalanceOutRequest $request
    ): JsonResponse {
        $validated = $request->validated();
        return $action->execute($validated);
    }
