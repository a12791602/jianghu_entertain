 ...
 */
function generateProbFormula(array $itemProbConfig): array
{
    $probMin      = 0;
    $probMax      = 0;
    $itemWithProb = [];
 ...
 */
function drawing(array $itemWithProb, int $possible_out_comes = 100, int $loop = 1)
{
    $lastItem = 0;
    for ($i = 0; $i < $loop; $i++) {
 ...
 * @return integer|string
 */
function getItemByProb(array $proArr)
{
    $arr_formula = generateProbFormula($proArr);
    return drawing($arr_formula, (int) array_sum($proArr));
}
